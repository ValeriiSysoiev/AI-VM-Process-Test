(() => {
  ...
  const fetchPlanDetails = async (session_id) => {
    ...
    return fetch(apiEndpoint + "/api/triage?session_id=" + session_id, {
      method: "GET",
      headers: headers,
    })
  ...
  const fetchTaskStages = (task) => {
    window.headers.then((headers) => {
      fetch(apiEndpoint + "/api/remediate?plan_id=" + task.id, {
        method: "GET",
        headers: headers,
      })
        .then((response) => response.json())
          ...
          if (data && data.length > 0) {
            ...
            data.forEach((stage) => {
              ...
              if (stage.human_approval_status === "requested")
                stageActions = `
                                    <div class="menu-stage-actions">
                                        <i title="Approve critical vulnerability" class="fa-solid fa-square-check ml-3 menu-stage-action has-text-info" data-action="approved" data-stage="${stageBase64}"></i>
                                        <i title="Reject stage" class="fa-solid fa-square-xmark ml-1 menu-stage-action" data-action="rejected" data-stage="${stageBase64}"></i>
                                    </div>
                                `;
              ...
              if (stage.status === "completed") {
                ...
                stage.status === "action_requested" ||
                stage.status === "awaiting_feedback"
              ) {
                statusClass = "is-info";
              } else if (stage.status === "failed" || stage.status === "rejected") {
              ...
              stageItem.innerHTML = `
                                <a class="menu-stage ${statusClass} ${stage.status}
  ...
  const fetchTaskMessages = (task) => {
    window.headers.then((headers) => {
      fetch(apiEndpoint + "/api/report?session_id=" + task.session_id, {
        method: "GET",
        headers: headers,
      })
        .then((response) => response.json())
        .then((data) => {
          const AGENT_NAME_MAP = {
            "Planner Agent": "Ingestion Agent",
            "Hr Agent": "Triage Agent",
            "Expense Billing Agent": "Remediation Agent",
            "Invoice Reconciliation Agent": "Reporting Agent",
            "Human Agent": "Security Analyst",
          };

          const toAgentName = (str) => {
            let new_name = str.replace(/_/g, " ");
            return AGENT_NAME_MAP[new_name] || new_name;
          };
              ...
              messages.forEach((message) => {
                ...
                if (
                  ...
                  if (showApproveAll) {
                    ...
                    approveAllStagesButton = `If you are happy with the plan, you can approve all remediation steps. <br/><button id="approveAllStagesButton" class="button mt-3 is-info">Approve all steps</button>`;
                  }
                }
                const messageLeft = `
  ...
  const actionStage = (action, stage) => {
    if (isHumanFeedbackPending()) {
      notyf.error(
        "You must first provide feedback to the AI-Powered Vulnerability Management."
      );
      return;
    }
    ...
    window.headers.then((headers) => {
      fetch(apiEndpoint + "/api/remediate", {
        method: "POST",
        headers: headers,
        body: JSON.stringify({
          step_id: stageObj.id,
